...
var DEEPSEEK_RULEBOOK_MANIFEST = {"skill":"SKILL_UI_Mermaid_Writer_v1.3.md","skillSha256":"523e6bb25d45e2c9871dd2b8a9b8439d09baa4b3515d29f7e16fffc853ba2584","specification":"UI控件交互图_Mermaid转写规范_v2.3.md","specificationSha256":"247f6c810ce24978dcd983c5b834795bb6e8d56f407c22c4376bc6294fc1811a"};
...
function deepSeekSystemPrompt() {
  return [
    ...
    'D. 节点名称必须以确定性结构链中的 Figma 实际名称为准。handoff 中的普通错拼不能覆盖节点清单；只有“明确命名映射”清单列出的 Figma → UE/运行时双端映射才允许替换实际名称。',
    '',
...
function deepSeekUserPrompt(msg, attempt) {
  ...
  var explicitNameMappings = Array.isArray(msg && msg.explicitNameMappings) ? msg.explicitNameMappings : [];
  var lines = [];
  if (mode === 'repair') {
  ...
  lines.push('5. 节点名称以结构链中的实际 Figma 名称为准。handoff 中即使存在反引号错拼，也不得直接复制为最终节点名：能唯一对应时改用实际名称，不能唯一对应时写入 evidence_gaps。');
  lines.push('6. 只有下方“明确命名映射”列出的双端映射才允许用 UE/运行时名称替换 Figma 实际名称；不得根据单个陌生名称或命名规律猜映射。');
  lines.push('7. 交接资料中的【手动测试修正】属于用户最新业务修正，但普通错拼不属于命名修正；未进入明确映射清单的名称不得创造新节点或父子层级。');
  lines.push('8. 若交接资料与结构链发生冲突，不得静默选择或自行补全：保留确定性的结构事实，并把无法唯一解决的业务冲突写入 evidence_gaps。');
  ...
  lines.push('');
  lines.push('【明确命名映射（唯一允许替换 Figma 实际节点名的例外）】');
  lines.push(explicitNameMappings.length ? JSON.stringify(explicitNameMappings, null, 2) : '[无；handoff 中所有节点引用必须回归结构链实际名称]');
  lines.push('');
